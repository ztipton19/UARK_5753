...
uark_enrollment <- uark_enrollment %>%
  mutate(
    y_lead = slide_dbl(
      enrollment,                # replace with your y variable name if different
      ~ if (length(.x) < 2) NA_real_ else .x[2],
      .before = 0,
      .after = 1
    )
  )

# 2) rho_1 from y_t and y_{t+1}
rho1_manual <- with(uark_enrollment, cor(enrollment, y_lead, use = "complete.obs"))
rho1_manual
```
